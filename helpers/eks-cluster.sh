...
/etc/eks/bootstrap.sh $cluster_name"
    ...
    NODE_GROUP_STATUS=$(aws --region="$region" eks describe-nodegroup \ 
        --cluster-name "$cluster_name" --nodegroup-name "$node_group_name" 2> /dev/null | jq -r  '.nodegroup | .status')
    echo "NODE_GROUP_STATUS = $NODE_GROUP_STATUS"
    if [ -n "$NODE_GROUP_STATUS" ] && [ "$NODE_GROUP_STATUS" == "DELETING" ]; then 
